...
function MetallicExplodingGrid({ scrollProgress, isDark }) {
    ...
    for (let x = 0; x < gridSize; x++) {
      for (let y = 0; y < gridSize; y++) {
        for (let z = 0; z < gridSize; z++) {
          ...
          temp.push({
            initialPos: new THREE.Vector3(x * totalSize - offset, y * totalSize - offset, z * totalSize - offset),
            isCenter,
            key: `${x}-${y}-${z}`,
          })
        }
  ...
  return (
    <group ref={groupRef}>
      {cubes.map((data) => (
        <RoundedBox key={data.key} args={[cubeSize, cubeSize, cubeSize]} radius={0.08} smoothness={6} position={data.initialPos}>
...
export default function ExplodedCube({ isDark }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 0, pointerEvents: 'none', height: '100dvh', width: '100%' }}>
      <Suspense fallback={null}>
        <Canvas dpr={[1, 2]} camera={{ position: [9, 9, 9], fov: 35 }} gl={{ alpha: false, antialias: true }}>
          ...
          <Float speed={1.4} rotationIntensity={0.3} floatIntensity={0.35}>
            <MetallicExplodingGrid scrollProgress={scrollProgress} isDark={isDark} />
          </Float>
          <Environment preset="studio" />
          <EffectComposer>
            <Bloom intensity={isDark ? 0.4 : 0.1} luminanceThreshold={1} mipmapBlur />
            <Noise opacity={0.02} />
          </EffectComposer>
        </Canvas>
      </Suspense>
